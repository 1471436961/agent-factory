...
当前仓库处于 **Alpha / M2**。M1 核心生产链已于 2026-07-21 通过项目 owner 人工验收并封存。M2.1-M2.5 已实现技能/评估契约、治理持久化、确定性评估、显式晋升、观察窗口与自动降级；M2.6 的治理 REST 路由、重启恢复主链和阶段退出验收尚未完成。
...
- 当前实现不接入真实 LLM，也不运行 Agent 任务；评估输入仍由外部提交。治理 REST、Gradio、多 Agent 协作和分布式基础设施不在当前已完成范围。
