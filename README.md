...
当前仓库处于 **Alpha / M4.6 本地退出候选已通过，远程 CI 待验证**。M1 核心生产链、M2 技能治理及 M3 接口、受限运行时与演示已经项目 owner 验收并封存；M4 已建立契约快照、安全回归、事务故障和隔离 wheel 真实进程证据，本地退出门禁全部通过。M4 仍不包含公网生产部署能力，只有远程 CI 通过并经项目 owner 人工确认后才会封存。
