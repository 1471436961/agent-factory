...
修正后的 candidate 包含原 62 项输入和 91 项生产文件。当时的 OpenAI canonical Manifest 绑定 source commit `e76adc778300b73b5973920fbaaa72275501db8d`，内部 checksum 为 `58afac123924e0604ec4067f0492781e7115a97b6c14900aee5bcff8fcd05713`，文件 SHA-256 为 `9758d465b44663baf18ced7f06ef51292d57037e1840c7dc17ba63fb94a1cecf`。该文件在任何 tracked 归档变化前通过环境级和 content-only 验证；供应商切换后原字节保留为 [`freeze-manifest-openai-pre-switch.json`](../../experiments/evidence/writer-pilot-v1/freeze-manifest-openai-pre-switch.json)。
...
原 OpenAI production-closure Manifest 已原样归档为 [`freeze-manifest-openai-pre-switch.json`](../../experiments/evidence/writer-pilot-v1/freeze-manifest-openai-pre-switch.json)。前两次真实 Pilot 使用的 Moonshot Manifest 从 clean source commit `889807a15b3d1cff9fe5df51f077de2110f6464a` 生成，现已原字节归档为 [`freeze-manifest-moonshot-pre-mfjs.json`](../../experiments/evidence/writer-pilot-v1/freeze-manifest-moonshot-pre-mfjs.json)。MFJS 修正后的 canonical [`freeze-manifest.json`](../../experiments/evidence/writer-pilot-v1/freeze-manifest.json) 绑定 clean source commit `e010b5356019e29aa4a89b4a10722671073589d5` 和 154 项输入；内部 checksum 为 `8b92ee21ce97611d9887ad5b2117f9f724c6ecd2c0609be26cf01c643302e17f`，文件 SHA-256 为 `75c0494161095dc64f566920b7fe232480237eef97bb8f8c40a24600ad02493d`，并在独立精确批准后用于第三次 8-run Pilot。该批准不延伸到正式 240-run。

## 29. M5.5 Moonshot Pilot 实证纠偏

项目 owner 先后批准两次真实 Moonshot Pilot，二者都使用 frozen source commit `889807a15b3d1cff9fe5df51f077de2110f6464a` 和相同 8-run 计划。第一次因本地凭据设置错误得到 8 个 `provider-failed`，证据保留在 `E:\Agent-Factory-Kimi-Pilot-Evidence`。第二次使用有效开放平台凭据，证据保留在 `E:\Agent-Factory-Kimi-Pilot-Evidence-Retry-20260726`；MANUAL 4/4 成功，FACTORY 4/4 为 `MOONSHOT_OUTPUT_NOT_JSON_OBJECT`，全部 run 都在第一次 attempt 终止。

第二次执行的旧 journal 只记录成功侧 1,416 input + 612 output token，对应 `CNY 25728 micros`。四个 FACTORY 失败的有界原始 chunk 仍包含一致 usage，合计 872 input + 49 output；完整可观察 usage 是 2,288 input + 661 output，按冻结价格重算为 `CNY 32719 micros`。executor 的调用前 reservation 仍按最多 16 次 attempt 和最坏 token 上限阻断超支，因此硬预算机制没有失效；需要修正的是失败 usage 的证据模型和观测成本汇总。

两组差异定位到 provider Structured Output 边界。MANUAL 使用 `json_object` 且 prompt 文本包含完整 Schema；FACTORY 将完整 Draft 2020-12 Schema 直接放入 `response_format.json_schema`。Moonshot [Structured Output 文档](https://platform.kimi.com/docs/api/chat) 只承诺 MFJS，[MFJS 规范](https://github.com/MoonshotAI/walle/blob/main/docs/mfjs-spec.zh.md) 不包含 Pilot Schema 使用的 `$schema`、`minLength`、`minItems` 和 `maxItems` 等关键字，也不保证对未知关键字稳定报错。四次 FACTORY 都返回短小非 JSON 内容，而非 API 参数错误，说明 mock 调用签名通过不能替代真实 provider 子集验证。

修正后的边界如下：

```text
完整 Draft 2020-12 Schema
        │
        ├─ deterministic MFJS projection ─► Moonshot response_format
        │      ├─ 保留受支持结构关键字
        │      ├─ 本地约束转为稳定 description hint
        │      └─ 未知结构关键字在网络调用前拒绝
        │
        └─ 原始 Schema ───────────────────► MANUAL/FACTORY 本地共同终验
```
...
本次实证和修正不新增 M5.5 子里程碑编号。旧 Manifest 继续绑定并解释前两次已执行请求；修正源码通过离线门禁并从 clean commit 重新冻结后，项目 owner 另行批准了仅限新 Manifest 的第三次真实 Pilot。M5.6 正式实验仍未授权。完整证据表见 [`M5.5 Moonshot Pilot 执行与纠偏报告`](../reports/m5.5-moonshot-pilot-review.md)。

## 30. M5.5 MFJS 兼容性复验

第三次 Pilot 使用 detached worktree `e010b5356019e29aa4a89b4a10722671073589d5`、canonical Manifest `8b92ee21ce97611d9887ad5b2117f9f724c6ecd2c0609be26cf01c643302e17f` 和独立证据根 `E:\\Agent-Factory-Kimi-Pilot-Evidence-MFJS-20260728`。执行前默认 verifier 对 source commit、干净工作树、CPython `3.11.15`、OpenAI SDK `2.46.0` 和 154 项文件执行 `content-and-environment` 校验；拒绝性预检证明省略 `--allow-live` 时不会读取密钥或创建证据目录。用户级密钥在启动时导入当前进程后立即删除，执行结束后进程级变量也被清除。

完整 8-run 于 `2026-07-28T06:49:19.809042Z` 至 `2026-07-28T06:50:55.890361Z` 执行。FACTORY 4/4 成功，MANUAL 3/4 成功；唯一失败是 MANUAL `brivane-recovery-brief` 的 `MOONSHOT_OUTPUT_NOT_JSON_OBJECT`。8 个 run 都在第一次 attempt 进入终态，全部 attempt 有 provider request ID。`ExperimentEvidenceLoader` 重新加载时验证了 8 个 request、8 个 attempt、8 个 terminal、计划顺序、execution manifest 和来源 checksum，没有发现缺失或计划外文件。
...
本轮只评审 MFJS 修正所针对的 provider 兼容性。FACTORY 从修正前 0/4 成功变为修正后 4/4 成功，故该门禁通过；MANUAL 单次失败说明 JSON mode 本身不提供逐次结构保证。协议没有预注册 8/8 成功阈值，本结果不能转化为 H1-H5、整体质量或条件优劣结论。在该检查点，M5.5 仍需冻结正式模型配置、预算和 Manifest；最终审批状态见第 31 节。
