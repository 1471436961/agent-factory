...
| 模块 | 职责 |
| --- | --- |
| `contracts.py` | M1/M2/M3.2 请求、健康检查和统一错误模型 |
...
| POST | `/api/v1/instances/{id}/transitions` | `TransitionInstanceRequest` | 200 `AgentInstance` | `factory:write` | 可选 |
...
transition 的 path `instance_id`、认证主体和幂等键分别由 URL、`Principal` 和 header 注入，客户端只能提交 expected revision、目标状态、原因和 retry 标记。通用 transition 不能把目标设为 `DEGRADED`；进入 `RUNNING` 前 Controller 会验证当前快照可构建 `AgentSpec`，但不会隐式持久化规格。状态 revision 变化后，调用方必须显式重新执行 spec export。
