...
M5.5.7 在 clean source commit `d3c19beb75587b5cc9963c05832c918694dfa9e1` 上完成最终冻结，绑定 CPython `3.11.15`、OpenAI SDK `2.46.0`、lockfile 和包含 `pilot_launcher.py` 的 62 项输入。canonical [`freeze-manifest.json`](../experiments/evidence/writer-pilot-v1/freeze-manifest.json) 的 Manifest checksum 为 `6514a01799af9b6585f4ff009ad11c887439a324200771d0cae479f28f630d22`，文件 SHA-256 为 `994f0d46557adeea77703849b0eb3978abe3d9fe89a1741c01b802ffcd2d2740`。生成后先在 clean 工作树通过环境级和 content-only 验证，再按原始字节归档；后续归档提交只做 content-only 回归，环境级复核需 checkout source commit。该 Manifest 满足进入真实 Pilot 审批的工程前置条件，但 Git 归档不是签名或外部对象锁，冻结也不等于费用审批、真实调用或质量结论。
