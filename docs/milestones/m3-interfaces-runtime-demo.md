...
该适配器只解决本地 Alpha 的最小信任来源，不承担用户目录、第三方身份联合、密钥生命周期或公网防护。M3 封存时原计划由 M4 补齐“完整安全与回归门禁”；M4.1 经人工 review 后已修正为当前 Alpha 的安全、回归与发布门禁，完整身份与公网防护进入后续 Productionization。
...
| 静态 Token 被误解为生产认证 | 文档标注 Alpha、禁止公网承诺、M4 验收当前边界 | 需要多用户、轮换、第三方身份或公网入口 |
